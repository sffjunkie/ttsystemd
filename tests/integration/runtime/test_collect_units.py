...
from ttsystemd.systemd.runtime.collect import dbus_collect_units


@pytest.mark.asyncio
async def test_int_units_system():
    result = await dbus_collect_units(SessionType.SYSTEM)
...
async def test_int_units_user_session():
    result = await dbus_collect_units(SessionType.USER_SESSION)
    assert len(result.units) > 0
